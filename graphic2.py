...
class BlackjackGUI(QMainWindow):
    ...
    def initUI(self):
        ...
        self.player_hand_label = QLabel("Player Hand 1: ")
        self.layout.addWidget(self.player_hand_label)
        self.player_hand2_label = QLabel("Player Hand 2: ")
        self.layout.addWidget(self.player_hand2_label)
        self.dealer_hand_label = QLabel("Dealer Hand: ")
        self.layout.addWidget(self.dealer_hand_label)

        # Buttons for actions
        self.actions_layout = QHBoxLayout()
        self.hit_button = QPushButton("Hit")
        self.hit_button.clicked.connect(lambda: self.take_action(0))
        self.actions_layout.addWidget(self.hit_button)
        self.stick_button = QPushButton("Stick")
        self.stick_button.clicked.connect(lambda: self.take_action(1))
        self.actions_layout.addWidget(self.stick_button)
        self.double_down_button = QPushButton("Double Down")
        self.double_down_button.clicked.connect(lambda: self.take_action(2))
        self.actions_layout.addWidget(self.double_down_button)
        self.split_button = QPushButton("Split")
        self.split_button.clicked.connect(lambda: self.take_action(3))
        self.actions_layout.addWidget(self.split_button)
    ...
    def update_display(self):
        ...
        self.player_hand_label.setText(f"Player Hand 1: {state['current_hand1']} (Sum: {state['current_sum1']})")
        self.player_hand2_label.setText(f"Player Hand 2: {state['current_hand2']} (Sum: {state['current_sum2']})")
        self.dealer_hand_label.setText(f"Dealer Hand: {state['current_dealer_hand']} (Sum: {state['current_dealer_sum']})")
        self.split_button.setEnabled(self.env.can_split and not self.env.done_split)

    def show_message(self, winner, reward):
        QMessageBox.information(self, "Game Over", f"Winner: {winner}, Reward: {reward}")
